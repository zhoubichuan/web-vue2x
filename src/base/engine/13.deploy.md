...
- 常用功能 **[例如：`dev`开发分支，`master`主分支，现在`dev`开发完成，需要合并到`master`]**
  - 代码提交：为防止有冲突产生，可以先将本地代码暂存；然后拉取代码，然后将暂存的代码合并，有冲突解决冲突后再提交
  - 创建分支
  - 切换分支：`git checkout dev`
  - 分支合并 **(先将master合并到dev,然后将dev合并到master)**：`git pull origin master` 、`git checkout dev` 、`git merge master`、`提交代码（有冲突解决冲突）`、`git push -u dev` 、`git checkout master` 、`git merge dev`
