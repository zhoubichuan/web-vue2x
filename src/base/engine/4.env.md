...
- dev（开发环境）开发环境是程序员们专门用于开发的服务器，配置可以比较随意， 为了开发调试方便，一般打开全部错误报告
- sit（测试环境）dev 环境完成需求开发就可以将代码打包到 sit 环境，给测试验证相关功能
- uat（客户体验环境）一般是克隆一份生产环境的配置，sit 测试没有问题就可以将代码打包到 uat 环境，然后由需求提出方验收需求
- prod（生产环境）uat 环境完成需求验收就可以将代码部署到正式环境
