...
- **publicPath**：可以配置二级目录，比如：`http://localhost:8080/vue`
...
- **outputDir**：配置文件打包输出到 output 文件夹下
...
- **productionSourceMap**：是否为生产环境构建生成 source map,一般在生产环境下为了快速定位错误信息，我们都会开启 source map
