...
export default {
  ...
  render: (h) => {
    ...
    return h(
      "div",// 要渲染的元素或组件，可以是一个 html 标签、组件选项或一个函数（不常用），该参数为必填项
      { //对应属性的数据对象，比如组件的 props、元素的 class、绑定的事件、slot、自定义指令等，该参数是可选的
        attrs: {
          id: "main",
        },
        class: {
      ...
      [childNode] //子节点，可选，String 或 Array,它同样是一个 h
    )
  },
}
```
...
function 设置可以使组件无状态和无实例，也就是没有 data 和 this 上下文。这样用 Render 函数返回虚拟节点可以更容易渲染，因为函数化组件只是一个函数，渲染开销要小很多。

组件需要的 data、props、slots、children、parent 都是通过上下文来传递的，比如 `this.level` 要改写为 `ctx.props.level`,`this.$slots.default` 改写为 `ctx.children`
...
功能类似`slot`,如果示例中的`Render`是用 v-for 生成的，也就是多个时，用一个 slot 是实现不了的，那时用 Render 函数就很方便了
...
某些场景可以用 is 特性来动态挂载组件。函数化组件主要适用于以下两个场景：
