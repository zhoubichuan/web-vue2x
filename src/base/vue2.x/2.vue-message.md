...
props 传递简单数据类型：子组件 props 为响应式数据，data 接收后失去响应

props 传递数组对象：子组件 props 为响应式数据，data 接收后由于指向同一个地址空间，仍然有响应式

props 传递函数：可以修改父组件相关逻辑，也可以修改其状态，向父组件传递数据

- **1.props 为响应式数据，父组件传递的数据修改，子组件会跟着修改**
...
- **2.props 可以修改，但是会报错，直接修改 props 是不规范的不建议**
...
- **3.props 被 data 接收后，父组件修改，data 数据不会响应变化**
...
- **4.props 被 data 接收后，可以使用 watch 监听 props 变化，重新赋值 data 数据**
