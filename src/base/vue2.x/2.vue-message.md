...
::: tip 前言
介绍 Vue2.x 中 10 种组件之间的通信方式，在开发时能找到最佳应用场景
:::
...
全局信息：用户的登录信息维护、通知信息维护等全局的状态和数据，通过`inject`获取，我们**把整个 app.vue 实例通过`porvide`对外提供**
...
app.vue 只会渲染一次，利用这个特性，很适合做一次性全局的状态数据管理，例如,我们将用户的登录信息保存起来。
