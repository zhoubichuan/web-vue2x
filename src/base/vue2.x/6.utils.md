...
export default {
    ...
    methods: {
        async getData() {
            let { data, code } = await this.$api.queryAll();
            if (code === 200) {
                this.tableData = data
            }
        },
        async handleConfirm() {
            ...
            if (this.isAdd) {
                let { data, code } = await this.$api.add(this.$qs.stringify(this.form));
                if (code === 200) {
                    this.$message.success("数据添加成功")
                    this.getData()
                }
            } else {
              let { data, code } = await this.$api.update(this.$qs.stringify(this.form));
                if (code === 200) {
        ...
        async handleEdit({ id }) {
            let { data, code } = await this.$api.query({id});
            if (code === 200) {
                this.dialogFormVisible = true
                this.isAdd = false
                this.isView = false
        ...
        async handleDel({ id }) {
            let { data, code } = await this.$api.delSingle(this.$qs.stringify({ id }));
            if (code === 200) {
